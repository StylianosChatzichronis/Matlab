...
while (test_start < n)
    ...
    % Calculating variances of the training set classes
    variances_negative = var(cell2mat(class_negative_diabetes(:,1:features)));
    variances_positive = var(cell2mat(class_positive_diabetes(:,1:features)));
    
    % Iterating between the test set data samples
    for test = 1:size(test_set, 1)
        % Calculating the 2 class probabilites
        negative_diabetes = naiveClassProbability(means_negative, variances_negative, test_set(test,1:features));
        positive_diabetes = naiveClassProbability(means_positive, variances_positive, test_set(test,1:features));
        
        % Checking which class probability is bigger to classify our sample
